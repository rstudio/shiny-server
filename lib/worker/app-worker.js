...
function launchWorker_p(appSpec, pw, endpoint, logFilePath, workerId) {
    ...
    return exists(appSpec.appDir).then(function (exists) {
        ...
        if (!appSpec.logAsUser) {
            ...
            return Q.resolve(fs_promises.open(logFilePath, "a", mode_1))
    ...
    function AppWorker(appSpec, endpoint, logStream, workerId, home) {
        ...
        try {
            ...
            Q.nfcall(fs.stat, appSpec.appDir)
                ...
                self.$proc.stdin.end(appSpec.appDir +
                    "\n" +
                ...
                stdoutSplit.on("data", function stdoutSplitListener(line) {
                    ...
                    if (line.match(/^Starting Shiny with process ID: '(\d+)'$/)) {
                        var pid = parseInt(line.match(/^Starting Shiny with process ID: '(\d+)'$/)[1]);
                        self.$pid = pid;
                        logger.trace("R process spawned with PID " + pid);
                    }
                    else if ((match = line.match(/^Shiny version: (\d+)\.(\d+)\.(\d+)(\.(\d+))?$/))) {
                        logger.trace("Using shiny version: " +
                            match[1] +
                            "." +
                            match[2] +
                            "." +
                        ...
                        logger.trace("Using R version: " + match[1] + "." + match[2] + "." + match[3]);
                    }
                    else if ((match = line.match(/^rmarkdown version: (\d+)\.(\d+)\.(\d+)(\.(\d+))?$/))) {
                        logger.trace("Using rmarkdown version: " +
                            match[1] +
                            "." +
                            match[2] +
                            "." +
