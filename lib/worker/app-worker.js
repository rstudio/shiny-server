...
function spawnUserLog_p(pw, appSpec, endpoint, logFilePath, workerId) {
  ...
  let mode = 0o660;
  if (typeof(appSpec.settings.appDefaults.logFileMode) === "number" ||
      typeof(appSpec.settings.appDefaults.logFileMode) === "string") {
    mode = appSpec.settings.appDefaults.logFileMode;
  }
  // Convert numeric mode to octal
  ...
  rm.on('close', function (code) {
    if (code != 0){
      var err = "Failed to create log directory: " + logDir +", " + mode;
      logger.error(err);
      prom.reject(err);
    }
