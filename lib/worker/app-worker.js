...
function createBookmarkStateDirectory(bookmarkStateDir, username) {
  ...
  // Make sure top-level bookmark state directory exists
  return Q.nfcall(fs.mkdir, bookmarkStateDir, '711')
  .then(function() {
    logger.info('Created bookmark state directory: ' + bookmarkStateDir);
    return Q();
  })
  .fail(function(err) {
    return Q.nfcall(fs.stat, bookmarkStateDir)
    .then(function(stat) {
      if (!stat.isDirectory()) {
        var msg = 'Bookmark state directory existed, was a file: ' + bookmarkStateDir;
        logger.error(msg);
        throw err;
      }
    })
    .fail(function(err2) {
      var msg = 'Bookmark state directory creation failed: ' + bookmarkStateDir;
      logger.error(msg);
      throw err2;
    });
  })

  // Make sure user's bookmark state directory exists
  .then(function() {
    return Q.nfcall(fs.mkdir, userBookmarkStateDir, '700');
  })
  .then(function() {
    var pw = posix.getpwnam(username);
    return Q.nfcall(fs.chown, userBookmarkStateDir, pw.uid, pw.gid);
  })
  .then(function() {
    logger.info('Created user bookmark state directory: ' + userBookmarkStateDir);
  })
  .fail(function(err) {
    logger.error('Error making/chowning user bookmark dir: ' + userBookmarkStateDir);

    return Q.nfcall(fs.stat, userBookmarkStateDir)
    .then(function(stat) {
      if (!stat.isDirectory()) {
        var msg = "User's bookmark state directory existed, was a file: " + userBookmarkStateDir;
        logger.error(msg);
        throw err;
      }
    })
    .fail(function(err2) {
      var msg = "User's bookmark state directory creation failed: " + userBookmarkStateDir;
      logger.error(msg);
      throw err2;
    });
  });
  ...
  try {
    ...
    .then(function(stat){
      ...
      return createBookmarkStateDirectory(
        ...
        appSpec.runAs,
        path.basename(appSpec.appDir) // TODO Add hash
      );
    })
    .then(function() {
