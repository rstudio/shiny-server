 ...
 */
function launchWorker_p(appSpec, endpoint, logFilePath, workerId, appLogger) {
  if (!appSpec.runAs)
  ...
  if (!pw.uid || !pw.gid){
    return Q.reject(new Error("The run_as user must have a user ID and group ID."));
  }

  var token = appLogger.getToken(logFilePath, pw.uid, pw.gid);
  var header = appLogger.formatHeader(token, logFilePath);
  ...
  return exists_p(appSpec.appDir).then(function(exists) {
    ...
    // Create the worker
    var worker = new AppWorker(appSpec, endpoint, workerId, 
      pw.home, header);
    return Q(worker);
  });
};
 ...
 */
var AppWorker = function(appSpec, endpoint, workerId, home, 
    logHeader) {
  ...
  try {
    ...
    .then(function(stat){
      ...
      self.$proc.stdin.end(
        ...
        logHeader + '\n' + 
        (process.env.SHINY_LOGGING_PORT || 8115) + '\n'
      );
...
(function() {
  ...
  this.kill = function() {
    ...
    var pid = this.$pid;
    if (!pid){
      logger.info("Trying to kill a process with no PID.")
      return;
    }
