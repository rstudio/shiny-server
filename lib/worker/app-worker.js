  ...
  try {
    ...
    .then(function(stat){
      ...
      self.$proc = child_process.spawn(executable, args, {
        ...
        cwd: appSpec.appDir,
        env: {
          SHINY_APP: appSpec.appDir,
          SHINY_PORT: endpoint.getAppWorkerPort(),
          SHINY_GAID: (appSpec.settings.gaTrackingId || ''),
          SHINY_SHARED_SECRET: endpoint.getSharedSecret(),
          SHINY_SERVER_VERSION: SHINY_SERVER_VERSION,
          WORKER_ID: workerId,
          MIN_R_VERSION: MIN_R_VERSION.join('.'),
          MIN_SHINY_VERSION: MIN_SHINY_VERSION.join('.')
        },
