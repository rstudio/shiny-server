...
function checkVersion(required, found){
  var pointer = 0;
  while (pointer < required.length && pointer < found.length){
    found[pointer] = parseInt(found[pointer]);
    required[pointer] = parseInt(required[pointer]);
    if (found[pointer] > required[pointer]){
      return true;
    } else if (found[pointer] < required[pointer]){
      return false;
    } 
    // else continue through loop
    pointer++;
  }
  // made it to the end of the arrays and never returned -- must be equal
  return true;
}
 ...
 */
var AppWorker = function(appSpec, endpoint, logStream, workerId) {
    ...
    this.$proc.stdout.pipe(split()).on('data', function(line){
      ...
      if (line.match(/^Starting Shiny with process ID: '(\d+)'$/)){
        ...
        if (match[1] == 0 && match[2] == 0 && match[3] == 0){
          // Shiny not installed in any accessible library.
          logger.error("Unable to find Shiny package.");
        } else{
          
          var version = [match[1], match[2], match[3]];
          if (match[5]){
            version.push(match[5]);
          }
          if (!checkVersion(MIN_SHINY_VERSION, version)){
            var err = "Outdated Shiny version found: " + 
                version.join(".") + ", must be at least " + 
                MIN_SHINY_VERSION.join(".");
            logger.error(err);
            self.$dfEnded.reject(err);
          }
        }
        ...
        logger.trace("Using R version: " + match[1] + "." + match[2] + 
            "." + match[3]);
        var version = [match[1], match[2], match[3]];
        if (!checkVersion(MIN_R_VERSION, version)){
          var err = "Outdated R version found: " + 
              version.join(".") + ", must be at least " + 
              MIN_R_VERSION.join(".");
          logger.error(err);
          self.$dfEnded.reject(err);
