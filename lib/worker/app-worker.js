...
var AppWorker = function(appSpec, listenPort, logStream) {
  ...
  try {
    ...
    var args = [
      ...
      appSpec.runAs,
      '-c',
      'SHINY_APP=' + bash.escape(appSpec.appDir) + ' ' +
      'SHINY_PORT=' + bash.escape(listenPort) + ' ' +
      (appSpec.settings.gaTrackingId ?
        'SHINY_GAID=' + bash.escape(appSpec.settings.gaTrackingId) + ' ' :
        '') +
      bash.escape(rprog) + " --no-save -f " + bash.escape(scriptPath)
    ];

    this.$proc = child_process.spawn('/bin/su', args, {
