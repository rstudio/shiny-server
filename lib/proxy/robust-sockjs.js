  ...
  this.RobustConn = function(conn, id) {
    ...
    this.set = function(conn){
      if (robustConn._conn) {
        ...
        conn.write("CONTINUE " + robustConn._pendingMsgId.toString(16).toUpperCase());
      ...
      conn.emit = function(type) {
        ...
        if (type === 'data') {
          var match = /^([\dA-F]+)#/.exec(arguments[1]);
          if (!match) {
            logger.warn("Invalid RobustSockJS message, no msg-id found");
            robustConn.close();
          }
          robustConn._pendingMsgId = parseInt(match[1], 16) + 1;
          arguments[1] = arguments[1].replace(/^([\dA-F]+)#/, "");
          doEmit.apply(null, arguments);

          conn.write("ACK " + robustConn._pendingMsgId.toString(16).toUpperCase());
