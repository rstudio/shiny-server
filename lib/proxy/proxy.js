...
function ShinyProxy(router, workerRegistry, options) {
  ...
  // - Ask the worker registry for an AppWorkerInfo for the given AppSpec.
  //   An AppWorkerInfo represents a handle to a specific R process. (If error,
  //   respond with error 500.)
  // - If the AppWorkerInfo has no proxy field, create it.
  // - Proxy the request.
  this.httpServer = http.createServer(function(req, res) {
    ...
    var appSpec = router.getAppSpec(req.url);
    if (!appSpec) {
    ...
    workerRegistry.getWorker_p(appSpec).then(
      function(appWorkerInfo) {

        // Ensures that the worker process will not be reaped while we use it.
        appWorkerInfo.acquire();

        // Save a reference to the appWorkerInfo on the response so that we can
        // call appWorkerInfo.release() when the request ends.
        res.appWorkerInfo = appWorkerInfo;

        if (!appWorkerInfo.proxy) {
          // Cache an HTTP proxy right on the appWorkerInfo.
          appWorkerInfo.proxy = createHttpProxy(appWorkerInfo);
          appWorkerInfo.exitPromise.fin(function(status) {
            appWorkerInfo.proxy.close();
            logger.trace('Port ' + appWorkerInfo.listenPort + ' proxy closed');
          });
        }

        logger.trace('Proxying request: ' + req.url);
        appWorkerInfo.proxy.proxyRequest(req, res, buffer);
      },
      function(err) {
        // TODO: Respond with error
        error500(req, res, 'Could not get worker');
      }
    ).done();
  });
  ...
  var sockjsServer = sockjs.createServer({
    sockjs_url: 'http://cdn.sockjs.org/sockjs-0.3.min.js', // TODO: make configurable
    prefix: '.*/__sockjs__',
    log: function() {}
  });
  ...
  sockjsServer.on('connection', function(conn) {
    ...
    var appSpec = router.getAppSpec(conn.url);
    if (!appSpec) {
    ...
    workerRegistry.getWorker_p(appSpec).then(function(awi) {
      ...
      var wsUrl = 'ws://127.0.0.1:' + appWorkerInfo.listenPort + '/';
  ...
  function createHttpProxy(appWorkerInfo) {

    var proxy = new http_proxy.HttpProxy({
      target: {
        host: '127.0.0.1',
        port: appWorkerInfo.listenPort
      }
    });
    ...
    function cleanupResponse(response) {
      if (response.appWorkerInfo)
        response.appWorkerInfo.release();
      delete response.appWorkerInfo;
    }
