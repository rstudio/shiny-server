...
function createServer(router, schedulerRegistry) {
  ...
  function connectToApp(conn, appSpec) {
    ...
    var wrk;
    try{
      // Can't specify the URL that we're requesting, but as long as it's not
      // "/", the scheduler will know that it's not the initialization of a 
      // new session. Just provide some text 'ws'.
    ...
    .then(function(awh) {
      ...
      wsClient = new websocket.Client(wsUrl, undefined, {
