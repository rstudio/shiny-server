...
function ShinyProxy(router, schedulerRegistry) {
  ...
  function createHttpProxy(appWorkerHandle) {
    ...
    // jcheng 07/26/2016: There used to be some event handlers here that turned
    // off keepalive for the shiny-server-to-shiny request, while keeping
    // keepalive turned on for the browser-to-shiny-server request. This was
    // needed because Node.js wasn't respecting keepalive when making requests
    // over Unix domain socket transports, but we don't use Unix domain sockets
    // anymore and TCP doesn't have the same problem. Nevertheless, if we need
    // to bring those event handlers back, the events to use are called, at the
    // time of this writing, proxy.on('proxyReq') and proxy.on('proxyRes').
    ...
    // Handles the case where the response takes so long that the requester
    // gives up and closes the socket. Without this handler, the app worker
    // handle gets leaked!
