...
(function() {
  /**
   * 
   * @param nestInApp if true, will nest the loaded configuration file
   *   inside of an 'application' block which uses the appSpec.appDir
   *   to provide the application's appDir parameter. If false, just 
   *   parses the file as it exists on disk.
   */
  this.readConfig_p = function(appSpec, nestInApp) {
    ...
    .then(function(confPath){
      var schemaPath = path.join(__dirname, '../router/shiny-server-rules.config');
      if (!nestInApp){
        var conf = config.read_p(
          confPath, 
          schemaPath);
      } else{
        // Modification of config.read_p which supplements with
        //   nestInApp, as necessary.
        var conf = Q.nfcall(fs.readFile, confPath, 'utf8')
        .then(function(cdata) {
          return Q.nfcall(fs.readFile, schemaPath, 'utf8')
          .then(function(sdata) {
            return schema.applySchema(config.parse(cdata, confPath), 
                config.parse(sdata, schemaPath));
          });
        });
      }
