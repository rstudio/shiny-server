...
function read_p(path, schemaPath) {
  ...
  .then(function(cdata) {
    ...
    .then(function(sdata) {
      return schema.applySchema(parse(cdata), parse(sdata));
    });
  });
}
...
function readSync(path, schemaPath) {
  var configData = parse(fs.readFileSync(path, 'utf8'));
  var schemaData = parse(fs.readFileSync(schemaPath, 'utf8'));
...
(function() {
  ...
  this.search = function(criteria, includeSelf) {
    ...
    if (includeSelf && predicate(this))
      results.push(this);
    return _.flatten(results.concat(
      _.map(this.children, function(child) {
        return child.search(predicate, true);
      })
    ));
  };
