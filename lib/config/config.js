...
function parseConfigData(data) {
  ...
  return ruleToConfig(root, null);
}

function ruleToConfig(rule, parent) {
  var config = new Config(parent, rule.getDirective(), rule.getArgs());
  config.children = _.map(rule.children, function(child) {
    return ruleToConfig(child, config);
  });
  return config;
};
...
(function() {
  /**
   * Return a single rule with the given criteria, if it exists. This scope's
   * children are searched first. If no match is found and inherit is true,
   * then recurse to the parent, if any.
   *
   * @param {*} criteria The rule's criteria. See search().
   * @param {Boolean} [inherit] Whether to search ancestor scopes too.
  ...
  /**
   * Return all the rules that match the given criteria AND are direct children
   * of this config scope.
   *
   * @param {*} criteria The rule's criteria. See search().
