...
var ssjsTemplate = fs.readFileSync(paths.projectFile('node_modules/shiny-server-js/dist/shiny-server.min.js'), 'utf-8');
...
app.use(connect_util.filterByRegex(
  ...
  function (req, res, next) {
    ...
    if (req.url === 'shiny-server.js'){
      ...
      res.end(renderedSSJS);
    } else if (req.url === 'sockjs-0.3.4.min.js' || req.url === 'shiny-server.css')
      assets(req, res);
    else
  ...
  .then(function(configRouter) {
    ...
    _.each(_.uniq(configRouter.disabledProtocols), function(p){
      if (!_.contains(allProtocols, p)){
        throw new Error("Unrecognized disabled protocol: '"+p+"'. Should be one of " + (prots.join(',')));
      }
