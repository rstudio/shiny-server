  ...
  this.$findShinyDir_p = function(pathname) {
    ...
    function isAppDir_p(dirPath) {
      ...
      fs.exists(dirPath, function(exists) {
        if (!exists)
          deferred.resolve({exists: false, isApp: false, isRmd: false});
        else {
          deferred.resolve(
            ...
            .then(
              function(entries) {
                ...
                return {exists: true, isApp: !!app, isRmd: !!rmd};
              },
              function(err) {
                if (err.code == 'ENOTDIR') {
                  return {exists: false, isApp: false, isRmd: false};
                }
                else {
                  logger.error('Error reading dir: ' + err.message);
                  return {exists: true, isApp: false, isRmd: false};
                }
              }
            )
          );
        ...
        .then(function(testDirResult) {
          if (!testDirResult.exists)
            ...
            if (testDirResult.isApp)
              type = 'shiny';
            if (testDirResult.isRmd)
              type = 'rmd';

            // terminate loop and return subpath
            return {res: subpath, type: type};
          }
        });
      },
      function(result) {
