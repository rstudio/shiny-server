...
// Serve .R files as text/R rather than letting the browser download them.
// send 1.x dropped its own `mime` re-export and delegates to the mime-types
// package, so this reaches in there instead. It works because send resolves
// the same hoisted mime-types instance we do -- which is why mime-types is a
// direct dependency rather than something we borrow transitively.
    ...
    return qutil.forEachPromise_p(
      ...
      function(subpath) {
        ...
        .then(function(testDirResult) {
          ...
          else if (!testDirResult.isApp && !testDirResult.isRmd && !testDirResult.isPyApp)
