 ...
 *   index.html is not found. Alternatively can be a regex. If a regex, the
 *   dirs that match will be excluded from the directory listing.
...
(function() {
  this.getAppSpec_p = function(req, res) {
    ...
    .then(function(subpath) {
      if (subpath.res && subpath.type === 'shiny') {
      ...
      } else if (subpath.res && subpath.type === 'rmd') {
        // TODO
      ...
      fs.exists(indexPath, function(exists) {
        if (exists) {
          ...
          if (!self.$dirIndex) {
            ...
            var filter = (_.isRegExp(self.$dirIndex)) ? self.$dirIndex : undefined;
            deferred.resolve(
              self.$autoindex_p(req, res, this_SendStream.path, filter)
            );
          }
        }
      });
  ...
  this.$findShinyDir_p = function(pathname) {
    ...
    /**
     * Returns true/false depending on whether or not the dir looks like 
     * an Rmd dir.
     * @param dirPath The directory to check to see if it contains a default
     *   Rmd file.
     * @returns A promise that resolves to false if there's an error accessing
     *   the directory or if it doesn't look like an Rmd dir. Returns the full
     *   path to the file (fronted by dirPath) if there does appear to be a 
     *   default Rmd file in this dir.
     **/
    function isRmdDir_p(dirPath) {
      return Q.nfcall(fs.readdir, dirPath)
      .then(function(entries) {
        var found = _.find(entries, function(entry) {
          return /^index\.rmd$/i.test(entry);
        });
        return (found ? path.join(dirPath, found) : false);
      },
      function(err) {
        if (err.code == 'ENOTDIR') {
          return false;
    ...
    .then(function(shinyApp){
      if (shinyApp === null){
        // Means we got through all the dirs without ever finding a Shiny app.
        // Because the previous promise didn't return false, we already know
        // that all the dirs exist.
        return isRmdDir_p(path.join(self.$root, pathname))
        .then(function(isrmd){
          if (!!isrmd){
            return {res: isrmd, type: 'rmd'};
          } 
          return {res: isrmd};
        });
      } else if (!!shinyApp){
        // Looks like there's a real shinyApp, supplement.
        return {res: shinyApp, type: 'shiny'};
      } else{
        // Just pass through if non-null falsey
        return {res: shinyApp};
      }
    })
    .fail(function(err) {
