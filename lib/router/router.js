...
function SingleAppRouter(appdir, runas, prefix, logDir, settings) {
  ...
  // Setup the initialization logic that sets the mode.
  this.$initPromise = Q.nfcall(fs.readdir, self.$appSpec.appDir)
    .then(function(files){ 
      ...
      if (app){
        self.$appSpec.settings.mode = 'shiny';
      } else if(rmd){
        self.$appSpec.settings.mode = 'rmd';
      } else{
        // No app found.
      }
    })
