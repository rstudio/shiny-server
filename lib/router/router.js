...
function AutouserRouter(prefix, groups, settings, applications) {
...
(function() {
  this.getAppSpec_p = qutil.wrap(function(req, res) {
    ...
    // Revise the settings based on a matching 'application' block.
    if (_.has(this.$applications, dir)){
      var appNode = this.$applications[dir];
      var appSettings = configRouterUtil.parseApplication({}, appNode, false);
      appConfig.merge(appSettings, this.$settings);
    }

    return new AppSpec(dir, username, prefix, logDir, this.$settings);
  });
