 ...
 **/
function AutouserRouter(prefix, groups, settings, runas) {
  prefix = prefix.replace(/\/$/m, ''); // strip trailing slash, if any
  this.$prefix = prefix;
  this.$groups = groups || [];
...
(function() {
  this.getAppSpec_p = qutil.wrap(function(req, res) {
    ...
    var username = m[1];
    var runas = username;
    if (this.$runas && this.$runas !== ':HOME_USER:'){
      // Then we are not to be running the app as the user specified in the
      // URL.
      runas = this.$runas;
    }
