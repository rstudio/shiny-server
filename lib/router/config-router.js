...
const userDb = require('../core/user-db');
...
function resolveRunAsUsers(users) {
  if (!users) return users;
  return users.map(function(user) {
    return user === ':PROCESS_USER:' ? permissions.getProcessUser() : user;
  });
}
...
function checkPermissions(conf) {
  ...
  // Listen nodes with ports under 1024. Port 0 means "pick an ephemeral port",
  // which is never privileged, so it must not be caught by this check.
  var listens = _.filter(conf.search('listen', false), function(node) {
    return node.values.port !== 0 && node.values.port < 1024;
  });
...
function createServers(conf) {
  ...
  var servers = _.map(conf.search('server'), function(serverNode) {
    ...
    if (typeof port === 'undefined')
      port = 80;
    // Port 0 means "let the OS pick an ephemeral port"; ask the bound server
    // via Server#addresses() to find out which one it got. Note that servers
    // are keyed by host/port, so two `listen 0` blocks on the same host would
    // collapse into a single listener -- fine for tests, which is what this is
    // for, but not something to rely on in a real config.
    if (port < 0)
...
function ServerRouter(port, host, vhosts, locations, templateDir) {
  // 0 is a legal port here -- it means an ephemeral port was requested. See
  // getScore, which treats it as matching any port.
...
(function() {
  ...
  this.getScore = function(req, res) {
    ...
    // `listen 0` asks the OS for an ephemeral port, so the config has no way
    // to name the port the request will arrive on; treat 0 as matching any.
...
function createSiteDir(locNode, node, settings) {
  ...
  var runas = resolveRunAsUsers(locNode.getValues('run_as').users);
...
function createUserApps(locNode, node, settings, userDirsMode) {
  ...
  var runas = null;
  if (userDirsMode)
    runas = resolveRunAsUsers(locNode.getValues('run_as').users);
  ...
  groups = _.map(groups, function(group) {
    try {
      var groupInfo = userDb.lookupGroup(group);
      if (!groupInfo) {
