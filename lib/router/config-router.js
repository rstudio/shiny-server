...
function createLocation(locNode) {
  ...
  var gaid = locNode.getValues('googleAnalyticsId').gaid;
  if (gaid)
  ...
  if (appdirPath) {
    ...
    return new router.SingleAppRouter(appdirPath, runas, path, logdir, settings);
  } else if (userappsEnabled) {
    return new router.AutouserRouter(path, settings);
