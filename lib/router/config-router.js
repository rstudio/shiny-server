...
/**
 * Check the provided locations to ensure that there aren't any more specific 
 * locations stored beneath a less specific location as a sibling.
 **/
exports.checkLocations = checkLocations;
function checkLocations(locations){
  var conflict;
  var prefixList = [];
  _.find(locations, function(loc){
    var thisPrefix = loc.prefix;
    if (!thisPrefix || thisPrefix === ''){
      thisPrefix = '/';
    }
    
    var isConflict = _.some(prefixList, function(pre){
      // We share a prefix with an existing location. Might indicate a 
      // problem.
      var con = thisPrefix.indexOf(pre) === 0;
      
      // Store the prefix elsewhere so we can refer to it later.
      if (con){
        conflict = pre;
      }
    ...
    prefixList.push(thisPrefix);
    return isConflict;
  });

  if (conflict){
    logger.warn("It looks like there may be a problem with your configured locations. " + 
      "Specifying a more specific location underneath a more general location can cause " + 
      "unexpected consequences. Consider putting the more specific location inside of " + 
      "the more general one. The location of concern is: '" + prefixList.pop() + "'");
  }
}
