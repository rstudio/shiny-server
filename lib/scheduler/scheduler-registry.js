...
var SchedulerRegistry = function() {
...
(function() {
  ...
  this.getWorker_p = function(appSpec, url, worker) {
    ...
    if (!this.$schedulers[key]){
      ...
      this.$schedulers[key] = new SimpleScheduler(appSpec);
      var self = this;
      this.$schedulers[key].on('vacantSched', function(appKey){
        // bubble this event up to others
        self.emit('vacantSched', appKey);

        // Delete this scheduler from the registry.
