...
class WorkerEntry extends EventEmitter {
  ...
  close() {
    this.closed = true;
    // The idle timer would otherwise stay armed for the full app_idle_timeout
    // after the entry is closed, only to fire "idletimeout" for a worker that
    // is already gone. Harmless but pointless -- and it keeps a short-lived
    // process (a test run) alive waiting for a timer nobody needs.
    global.clearTimeout(this.timer);
    this.timer = null;
