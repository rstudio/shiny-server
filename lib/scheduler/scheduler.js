...
(function() {
  ...
  this.freeSock = function(sockName) {
    ...
    if (_.size(this.$workers) == 0){
      ...
      this.emit('vacantSched', this.$appKey);
      //TODO: pretty ugle using the global process eventEmitter. But Trying
      // to get back to the app-config would be a doozy, so I'll do this
      // until we can figure something out that's cleaner.
      process.emit('vacantSched', this.$appKey);
    }
