...
// From the native addon rather than hardcoded: these are struct flock values
// and they are not the same everywhere. F_WRLCK is 1 on Linux but 3 on
// macOS/BSD, where 1 is F_RDLCK -- so the previously hardcoded 1 asked for a
// *shared* lock off Linux, and two instances could hold it at once.
// A stale build/Release/posix.node predates these exports, in which case they
// are undefined -- and undefined coerces to 0 in the addon, which is a
// different (or invalid) lock type rather than an error. Fail where the cause
// is legible instead.
if (typeof posix.F_WRLCK !== 'number' || typeof posix.SEEK_SET !== 'number') {
  throw new Error(
    'build/Release/posix.node is out of date: it does not export the fcntl ' +
    'lock constants. Run `npm rebuild`.');
}

var F_WRLCK = posix.F_WRLCK;
var SEEK_SET = posix.SEEK_SET;
